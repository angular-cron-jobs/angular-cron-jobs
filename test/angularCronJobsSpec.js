describe('AngularCronJobs', function () {
  var $compile, $rootScope, cronService, $controller, cronFrequency;

  beforeEach(module('angular-cron-jobs'));


  beforeEach(inject(function (_$compile_, _$rootScope_, _cronService_, _$controller_, _cronFrequency_) {
    $compile = _$compile_;
    $rootScope = _$rootScope_;
    cronService = _cronService_;
    $controller = _$controller_;
    cronFrequency = _cronFrequency_;
  }));
  ...
  it("cron with init '35 23 29 4 *' should have minutes and hours set and base mode 6", function () {
    expect(cronService.fromCron('35 23 29 4 *')).toEqual({
      base: 6,
      minuteValue: 35,
      hourValue: 23,
      dayOfMonthValue: 29,
  ...
  it("cron with init '35 23 29 4 *' should have minutes and hours set and base mode 6. values should be array in multimode", function () {
    expect(cronService.fromCron('35 23 29 4 *', true)).toEqual({
      base: 6,
      minuteValue: [35],
      hourValue: [23],
      dayOfMonthValue: [29],
  ...
  it("cron with init '35,40,55 23 29 4 *' should have minutes (35, 40 & 55) and hours set and base mode 6", function () {
    expect(cronService.fromCron('35,40,55 23 29 4 *', true)).toEqual({
      base: 6,
      minuteValue: [35, 40, 55],
      hourValue: [23],
      dayOfMonthValue: [29],
      monthValue: [4]
    });
  ...
  it("cron should disallow minute if set in config", function () {
    ...
    var ctrl = $controller('CronJobInputController', {
      $scope: scope,
      cronFrequency: cronFrequency,
      cronService: cronService
    }, data);

    expect(ctrl.frequency[0].label).toEqual('Hour');
